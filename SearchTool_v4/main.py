...
def readPlan():
    ...
    fileName = "./SearchTool_v4/" + input("Enter the file name of experimental setting: ")
...
def createOptimizer(parameters): ###
    ...
    pType = parameters['pType']
    limitStuck = parameters['limitStuck']

    alg = eval(dictOptimizer.get(aType))

    alg.setAlgorithm(dictOptimizer.get(aType).replace("()", ""))

    # alg.setVariables(pType, aType, limitStuck)
